...
class OciComputeOps(object):
    ...
    INSTANCE_STOP = "STOP"
    _INSTANCE_STATE_MAP = {
        INSTANCE_START: oci.core.models.Instance.LIFECYCLE_STATE_RUNNING,
    ...
    def __init__(self, resource_config, logger):
        ...
        self.compute_client_ops = oci.core.ComputeClientCompositeOperations(
            self.compute_client
        )
    ...
    def change_instance_state(self, instance_id, new_state):
        ...
        if instance.data and instance.data.lifecycle_state != instance_state:
            self.compute_client_ops.instance_action_and_wait_for_state(
                instance_id, new_state, [instance_state]
            )

    def launch_instance(
        self, app_name, vm_details, ssh_pub_key, timeout=MAX_INSTANCE_WAIT_TIMEOUT
    ):

        compatible_shapes = pagination.list_call_get_all_results(
        ...
        is_shape_compatible = next(
            (x.shape for x in compatible_shapes.data if x.shape == vm_details.vm_shape),
            None,
        )
        if not is_shape_compatible:
            raise OciShellError(
                "Incompatible shape chosen for deployment on App {}. "
                "Please check the available shapes for this image on OCI".format(
                    app_name
                )
            )
        ...
        if (
            vm_details.primary_subnet.private_ip
            and vm_details.primary_subnet.private_ip.ip  # noqa #W503
        ):
            new_inst_details.create_vnic_details.private_ip = (
                vm_details.primary_subnet.private_ip.ip
            )
        ...
        if vm_details.cloud_init_params:
            ...
            try:
                ...
                pass
            new_inst_details.metadata["user_data"] = base64.b64encode(
        ...
        try:
            launch_instance_response = (
                self.compute_client_ops.launch_instance_and_wait_for_state(
                    new_inst_details, wait_for_states=wait_for_states
                )
            )
            return launch_instance_response.data
        except CompositeOperationError as e:
            deployed_instance = next(
                i.data for i in e.partial_results if i.data.display_name == app_name
            )
            self._logger.error(
                "Failed to launch instance {}".format(deployed_instance.display_name)
            )
            instance = self.compute_client.get_instance(deployed_instance.id)
            if (
                instance.data.lifecycle_state
                == oci.core.models.Instance.LIFECYCLE_STATE_RUNNING  # noqa #W503
            ):
                return instance.data
            elif (
                instance.data.lifecycle_state
                == oci.core.models.Instance.LIFECYCLE_STATE_STARTING  # noqa #W503
            ):
                try:
                    waiter_result = oci.wait_until(
                        ...
                        and getattr(r.data, "lifecycle_state").lower()  # noqa #C412
    ...
    def get_windows_credentials(self, instance_id):
        try:
            instance_credentials = (
                self.compute_client.get_windows_instance_initial_credentials(
                    instance_id
                )
            )
    ...
    def get_vnic_attachments(self, instance_id):
        """Retrieve all vnics attached to a certain instance.
    ...
    def terminate_instance(self, instance_id=None, timeout=5, retries=5):
        ...
        try:
            instance = self.compute_client.get_instance(instance_id)
            if (
                not instance  # noqa #W503
                or instance.data.lifecycle_state  # noqa #W503
                == oci.core.models.Instance.LIFECYCLE_STATE_TERMINATED  # noqa #W503
            ):
                return
        ...
        try:
            self.compute_client_ops.terminate_instance_and_wait_for_state(
                instance_id, [oci.core.models.Instance.LIFECYCLE_STATE_TERMINATED]
            )
            ...
            for result in ce.partial_results:
                if result.data.id == instance_id:
                    ...
                    while (
                        ...
                        time.sleep(timeout)
                        instance_state = self.compute_client.get_instance(
                            instance_id
                        ).data.lifecycle
    ...
    def remove_vnic(self, vnic_id):
        try:
            self.compute_client_ops.detach_vnic_and_wait_for_state(
                vnic_id, [oci.core.models.VnicAttachment.LIFECYCLE_STATE_DETACHED]
            )
            ...
            for result in c_e.partial_results:
                if (
                    result
                    and result.data  # noqa #W503
                    and result.data.id == vnic_id  # noqa #W503
                    and result.data.lifecycle  # noqa #W503
                    != oci.core.models.VnicAttachment.LIFECYCLE_STATE_DETACHED  # noqa #W503
                ):
                    self.compute_client_ops.detach_vnic_and_wait_for_state(
                        vnic_id,
    ...
    def create_instance_console(self, instance_id, ssh_pub_key, compartment_ocid, tags):
        ...
        result = None
        instance_console_id = next(
            (console_instance for console_instance in instance_console_ids.data), None
        )
        if instance_console_id:
            result = self.compute_client.get_instance_console_connection(
                instance_console_id.id
            )
        if not result:
            result = self.compute_client_ops.create_instance_console_connection_and_wait_for_state(  # noqa: E501
                create_console,
                wait_for_states=[
                    oci.core.models.InstanceConsoleConnection.LIFECYCLE_STATE_ACTIVE
                ],
            )
        return result
    ...
    def create_image_from_instance(self, instance_ocid, compartment_ocid):
        ...
        if create_image_response.data:
            return {
                "OCI Shell.OCI VM from Image.Image ID": create_image_response.data.id
            }
        else:
            raise RuntimeError(
                "Timeout when waiting for new Image to reach state 'Available'"
            )
