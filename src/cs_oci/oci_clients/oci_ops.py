...
class OciOps(object):
    ...
    def __init__(self, resource_config, logger):
        ...
        self.storage_client = oci.core.BlockstorageClient(
        ...
        self.storage_client_ops = oci.core.BlockstorageClientCompositeOperations(
            self.storage_client
        )
        self.object_storage_client = oci.object_storage.ObjectStorageClient(config)
        self.object_storage_client_ops = (
    ...
    def get_vnic_attachments(self, instance_id):
        ...
        while not vnic_attachments and i != self.compute_ops.VNIC_ATTACHMENT_RETRY:
            ...
            vnic_attachments = [
                VNIC(oci_ops=self, logger=self._logger, vnic_attachment=vnic_att)
    ...
    def get_primary_vnic(self, instance_id, retries=6, timeout=5):
        ...
        while not result and i != retries:
            ...
            for attachment in attachments:
                ...
                self._logger.info(
                    "Attached vnic is {} is primary {}".format(
                        attachment.oci_vnic.display_name, attachment.oci_vnic.is_primary
                    )
                )
    ...
    def attach_secondary_vnic(
        ...
        result = self._attach_secondary_vnic(
            name, subnet_id, instance_id, is_public, private_ip, src_dst_check
        )
        i = 0
    ...
    def get_availability_domain_name(self):
        ...
        if self.resource_config.availability_domain:
            if any(
                x
                for x in list_availability_domains_response.data
            ...
            try:
                ...
                self._logger.exception("Unable to get proper Availability Domain")
    ...
    def create_ssh_keys_storage(self):
        ...
        return self.object_storage_client.create_bucket(
            namespace, create_bucket_details
        )
    ...
    def upload_keypairs(self, private_key, public_key):
        ...
        self.object_storage_client.put_object(
            namespace, bucket, self.resource_config.reservation_id, private_key
        )
        self.object_storage_client.put_object(
            namespace, bucket, self.resource_config.reservation_id + ".pub", public_key
        )
