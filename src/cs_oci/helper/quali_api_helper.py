...
class QualiAPIHelper(object):
    ...
    def login(self):
        ...
        self._session.headers.update(
            authorization="Basic {0}".format(result.replace('"', ""))
        )
    ...
    def attach_file_to_reservation(self, reservation_id, file_data, file_name):
        ...
        response = self._session.post(
            self._build_url(uri), data=data, files=file_to_upload, verify=False
        )
    ...
    def _valid(self, response):
        if response.status_code in [200, 201, 204]:
            ...
            raise RestClientUnauthorizedException(
                self.__class__.__name__, "Incorrect login or password"
            )
        else:
    ...
    def remove_attached_files(self, reservation_id):
        ...
        for file_name in self.get_attached_files(reservation_id) or []:
            ...
            response = self._session.post(
                self._build_url(uri), json=file_to_delete, verify=False
            )
