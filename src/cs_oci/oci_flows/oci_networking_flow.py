...
class OciNetworkInfraFlow(object):
    def __init__(self, oci_ops, logger, resource_config):
        """OCI Networking Infrastructure preparation flow.
    ...
    def prepare_sandbox_infra(self, request_object):
        ...
        for vcn_request in request_object.vcn_list:
            if vcn_request.is_main:
                ...
                if vcn_request.subnet_list:
                    ...
                    if any(x for x in vcn_request.subnet_list if x.attributes.public):
                        add_inet_gw = True
                    vcn = self._oci_ops.network_ops.create_vcn(
                        vcn_request.vcn_cidr, add_inet_gw=add_inet_gw
                    )
                    prepare_network_result.securityGroupId = (
                ...
                oci_vcn_name = "{}-{}".format(
                    self._resource_config.reservation_id, vcn_subnet.alias
                )
                vcn = self._oci_ops.network_ops.create_vcn(
                    vcn_subnet.cidr, oci_vcn_name, is_public
                )
            ...
            result_list.extend(
                self.create_subnets(
                    vcn_request.subnet_list, vcn.id, availability_domain
                )
            )
        vcn_dict = self._convert_list(traffic_vcn_list)
        for vcn in vcn_dict:
            for dst_vcn in vcn_dict.get(vcn):
                self._oci_ops.network_ops.allow_local_traffic(
                    vcn.default_security_list_id, dst_vcn.cidr_block
                )
                self._oci_ops.network_ops.allow_local_traffic(
                    dst_vcn.default_security_list_id, vcn.cidr_block
                )
    ...
    def create_subnets(self, subnet_list, vcn_id, availability_domain):
        ...
        for subnet_request in subnet_list:
            route_table = self._oci_ops.network_ops.create_route_table(
                vcn_id, subnet_request.alias
            )
            subnet = self._oci_ops.network_ops.create_subnet(
                subnet_request.cidr,
            ...
            if subnet_request.attributes.public:
                self._oci_ops.network_ops.add_inet_gw_route(
                    route_table.id, vcn_id=vcn_id
                )
